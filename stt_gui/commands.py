...
that only track, store and report the initial and final state, and can be
naturally integrated into Qt's Undo/Redo API. They become necessary when
performing interactive editings on big datastructures like pixmaps,  to prevent
memory bloating.
"""
...
class UndoableLambda(QtWidgets.QUndoCommand):
    """
    This kind of functor can be used to create commands on the spot and send
    them to the UndoStack. Useful to split down a composite action in arbitrary
